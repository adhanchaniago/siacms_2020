...
class Ekstrakurikuler extends CI_Controller {
	function __construct(){
		...
		if (false && ($this->session->userdata('jabatan') != 'Kurikulum')&&($this->session->userdata('jabatan') != 'Superadmin')&&($this->session->userdata('jabatan') != 'Kesiswaan')&&($this->session->userdata('jabatan') != 'Guru')&&($this->session->userdata('jabatan') != 'Pegawai')&&($this->session->userdata('jabatan') != 'Kepsek')&&($this->session->userdata('jabatan') != 'Ekskul')) {
			$this->session->set_flashdata("warning",'<script> swal( "Anda Tidak Berhakx!" ,  "Silahkan Login dengan Akun Anda" ,  "error" )</script>');
...
// tutup anggrek
	...
	public function ekstrakurikuler($id_jadwal_ekskul = "")
	{
		...
		if ($id_jadwal_ekskul == "" ) {
			...
			$this->template->load('kurikulum/dashboard','ekstrakurikuler/ekstrakurikuler', $data);
		} else {
			$this->load->model('penjadwalan/mod_jadwalekskul');
			$data['tabel_jadwalekskul'] = $this->mod_jadwalekskul->get();
			...
			$this->template->load('ekstrakurikuler/dashboard','ekstrakurikuler/ekstrakurikuler', $data);
		}
	}
