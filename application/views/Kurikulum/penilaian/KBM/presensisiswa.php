...
<body class="hold-transition skin-blue sidebar-mini">
                            ...
                            <table class="table table-bordered table-striped" style="width: 100%;border:0;">
                              <thead>
                                <tr class="barishari" style="background: #1e88e5;color: #eceff1;">
                                  <th class="fit" style="border: 0;border-radius: 1.5em 0 0 0;" rowspan="2">No</th>
                                  <th style="border: 0;" rowspan="2">Nama Siswa</th>
                                  <th style="border: 0;" rowspan="2">NISN</th>
                                  <?php
                                ...
                                <tr class="barishari" style="background: #1e88e5;color: #eceff1;">
                                    ...
                                    for($i=1;$i<=cal_days_in_month(CAL_GREGORIAN, $bln, $thn);$i++) { ?>
                                      <th style="border: 0">
                                          <select class="kodeguru presensi_trigger" data="<?php echo $i; ?>" name="presensi_name_<?php echo $i; ?>">
                                            <option value="">-</option>

                                            <?php if($check['s'] === 'on'):?>
                                              <option value="S">S Semua</option>
                                            <?php endif;?>

                                            <?php if($check['i'] === 'on'):?>
                                              <option value="I">I Semua</option>
                                            <?php endif;?>

                                            <?php if($check['a'] === 'on'):?>
                                              <option value="A">A Semua</option> 
                                            <?php endif;?>

                                            <?php if($check['h'] === 'on'):?>
                                              <option value="H">H Semua</option>
                                            <?php endif;?>
                                          </select>
                                        </th>
                                      <?php
                            ...
                            //for($i=1;$i<=date('t');$i++) {
                                ...
                                //echo @$libur[$thn][ltrim($bln, "0")][ltrim($i, "0")];
                                        if ((@$libur[$thn][ltrim($bln, "0")][ltrim($i, "0")] == "") && (@$liburnasional[ltrim($bln, "0")][ltrim($i, "0")] == "")) {
                                          ?>
                                          <select class='kodeguru select_ke_<?php echo $i ?>'  name="presensi_<?php echo $rowsiswa->nisn; ?>_<?php echo $i; ?>">
                                           <option value="">-</option>
                                           <?php if($check['h'] === 'on'):?>
  ...
  <script type="text/javascript">
    ...
    $('.presensi_trigger').on('change', function() {
      const value = this.value
      const data_id = $(this).attr('data')
      
      $(`.select_ke_${data_id}`).val(value)
    })
